...
Write-Host "Moving cursor to draw text '$Text' for 2 iterations..."
...
function Draw-LetterA {
    ...
    Move-CursorLine -startX $leftX -startY $bottomY -endX $centerX -endY $topY -steps 15
    # 2. Right diagonal (top to bottom right)
    Move-CursorLine -startX $centerX -startY $topY -endX $rightX -endY $bottomY -steps 15
    # 3. Middle line (horizontal bar)
    $middleLeftX = $centerX - ($letterWidth / 4)
    $middleRightX = $centerX + ($letterWidth / 4)
    Move-CursorLine -startX $middleLeftX -startY $middleY -endX $middleRightX -endY $middleY -steps 10
}
...
function Draw-LetterD {
    ...
    $topY = $offsetY
    $bottomY = $offsetY + $letterHeight
    $curveX = $offsetX + ($letterWidth * 0.7)
    
    # 1. Vertical line (left side)
    Move-CursorLine -startX $leftX -startY $topY -endX $leftX -endY $bottomY -steps 15
    # 2. Top curve
    Move-CursorLine -startX $leftX -startY $topY -endX $curveX -endY $topY -steps 10
    # 3. Right curved line
    Move-CursorLine -startX $curveX -startY $topY -endX $curveX -endY $bottomY -steps 15
    # 4. Bottom curve back
    Move-CursorLine -startX $curveX -startY $bottomY -endX $leftX -endY $bottomY -steps 10
}
...
# Function to draw a letter based on character
...
for ($iteration = 0; $iteration -lt 2; $iteration++) {
    Write-Host "Iteration $($iteration + 1) of 2"
